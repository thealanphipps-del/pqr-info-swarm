...
const PQR_API_URL = process.env.PQR_API_URL || 'http://localhost:8080/REST/2.0';
...
function activate(context) {
        ...
        try {
            switch (name) {
                case "create_pqr_ticket":
                    const createResp = await axios_1.default.post(`${PQR_API_URL}/ticket`, args);
                    return { content: [{ type: "text", text: JSON.stringify(createResp.data, null, 2) }] };
                case "query_pqr_memory":
                    const queryResp = await axios_1.default.get(`${PQR_API_URL}/agent/${args?.agentID || 'antigravity'}/context?limit=${args?.limit || 10}`);
                    return { content: [{ type: "text", text: JSON.stringify(queryResp.data, null, 2) }] };
                case "link_pqr_tickets":
                    const linkResp = await axios_1.default.post(`${PQR_API_URL}/ticket/${args?.parentID}/link/${args?.childID}`, {
                ...
                case "sync_github_issue":
                    const { ticketID, owner, repo, issueNumber } = args;
                    ...
                    // Update PQR ticket
                    const syncResp = await axios_1.default.put(`${PQR_API_URL}/ticket/${ticketID}`, {
                        Status: pqrStatus,
                        Title: issue.data.title
                    });
                    return { content: [{ type: "text", text: `Synced with GitHub #${issueNumber}: Status set to ${pqrStatus}` }] };
                case "create_healing_ticket":
                    const healingResp = await axios_1.default.post(`${PQR_API_URL}/healing/ticket`, request.params.arguments);
                    return { content: [{ type: "text", text: JSON.stringify(healingResp.data, null, 2) }] };
                case "process_healing_iteration":
                    const iterResp = await axios_1.default.post(`${PQR_API_URL}/healing/iterate/${request.params.arguments?.ticketID}`);
                    return { content: [{ type: "text", text: JSON.stringify(iterResp.data, null, 2) }] };
                case "record_healing_failure":
                    const failResp = await axios_1.default.post(`${PQR_API_URL}/healing/failure`, request.params.arguments);
                    return { content: [{ type: "text", text: JSON.stringify(failResp.data, null, 2) }] };
                case "resolve_healing_ticket":
                    const resResp = await axios_1.default.post(`${PQR_API_URL}/healing/resolve`, request.params.arguments);
    ...
    let disposable = vscode.commands.registerCommand('pqr.openHUD', () => {
        const panel = vscode.window.createWebviewPanel('pqrHUD', 'PQR Swarm HUD', vscode.ViewColumn.One, { enableScripts: true });
    ...
    let vaultSetup = vscode.commands.registerCommand('pqr.setupVault', async () => {
        ...
        const auth = await vscode.window.showInformationMessage(`Authorize sweeping secrets from ${path.basename(envPath)} into PQR Vault? This will securely migrate your credentials.`, { modal: true }, 'Authorize & Sweep');
        if (auth === 'Authorize & Sweep') {
            try {
                ...
                await axios_1.default.post(`${VAULT_ADDR}/v1/secret/data/pqr`, { data: secrets }, {
