...
export const SlingshotMixer: React.FC = () => {
  ...
  const [tracks, setTracks] = useState<TrackInfo[]>([
    { id: 'trk-1', name: 'CH 1', service: 'SRRK Router', sequenceStart: '0x00A1', sequenceEnd: '0x09FF', volume: 80, activity: 0, baseFreq: 220 }, // A3
    { id: 'trk-2', name: 'CH 2', service: '5D Manifold', sequenceStart: '0x1B22', sequenceEnd: '0x2C4A', volume: 65, activity: 0, baseFreq: 329.63 }, // E4
    { id: 'trk-3', name: 'CH 3', service: 'Q-State Ledger', sequenceStart: '0xF400', sequenceEnd: '0xFFFF', volume: 90, activity: 0, baseFreq: 440 }, // A4
    { id: 'trk-4', name: 'CH 4', service: 'Midi Handshake', sequenceStart: '0x33A1', sequenceEnd: '0x33B9', volume: 40, activity: 0, baseFreq: 554.37 }, // C#5
  ]);
  ...
  const playSynthBeep = (freq: number, volumeLevel: number, activityLevel: number) => {
    if (!audioCtxRef.current || !powerOn) return;
    
    // Only play if activity is high enough (simulating packet burst)
    if (activityLevel < 70) return;

    const osc = audioCtxRef.current.createOscillator();
    const gainNode = audioCtxRef.current.createGain();
    
    // Crossfader affects filter/pitch (Offline Cache = lowpass, Live = highpass vibes)
    osc.type = crossfader < 50 ? 'sine' : 'square';
    
    // Frequency modulation based on activity
    osc.frequency.setValueAtTime(freq + (activityLevel / 2), audioCtxRef.current.currentTime);
    
    // Volume scaling
    const vol = (volumeLevel / 100) * (activityLevel / 100) * 0.1; // keep it quiet
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setTracks(prev => prev.map(track => {
        ...
        if (powerOn) {
          playSynthBeep(track.baseFreq, track.volume, newActivity);
        }
