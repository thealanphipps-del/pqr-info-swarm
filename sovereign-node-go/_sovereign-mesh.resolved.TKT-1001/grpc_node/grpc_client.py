...
def run_suite(host, port, args):
    ...
    # L. SWARM LEDGER AUDIT
        ...
        if not grpc_online:
            ...
            ssh_py_cmd = """python3 -c "import sqlite3, json; conn=sqlite3.connect('/home/aellok/sovereign-mesh/agent_pedigree.db'); cur=conn.cursor(); cur.execute('SELECT block_index, previous_hash, timestamp, agent_id, mutation_payload, block_hash FROM ledger ORDER BY block_index ASC'); rows=cur.fetchall(); print('\\n--- SWARM LEDGER AUDIT VIA SSH FALLBACK ---'); [print('Block #' + str(r[0]) + ' | Proposer: ' + r[3] + ' | Time: ' + r[2] + '\\n  Prev: ' + r[1][:16] + '... | Hash: ' + r[5][:16] + '...\\n  Payload: ' + r[4]) for r in rows]" """
    ...
    # N. SWARM FORENSIC DECISION ACCOUNTING
        ...
        if not grpc_online:
            ...
            ssh_py_cmd = f"""python3 -c "import sqlite3, json; conn=sqlite3.connect('/home/aellok/sovereign-mesh/agent_pedigree.db'); cur=conn.cursor(); cur.execute('SELECT block_index, timestamp, agent_id, mutation_payload, consensus_votes, block_hash FROM ledger ORDER BY block_index ASC'); rows=cur.fetchall(); print('\\n🕵️ --- SWARM FORENSICS AUDIT VIA SSH ---'); [print('Block #' + str(r[0]) + ' | Proposer: ' + r[2] + '\\n  Time: ' + r[1] + '\\n  Hash: ' + r[5] + '\\n  Payload: ' + r[3] + '\\n  Votes: ' + r[4] + '\\n' + '='*80) for r in rows]; cur.execute('SELECT * FROM master_knowledge'); km=cur.fetchall(); print('\\nMaster Knowledge Dump:'); [print('  ' + k[0] + ' = ' + k[1] + ' (by ' + k[2] + ')') for k in km]" """
