...
1.  **[Dockerfile](file:///home/aellok/sovereign-mesh/Dockerfile):** Builds the CUDA-enabled training image based on the `pytorch/pytorch` stack, installing all telemetry dependencies.
2.  **[train_sovereign.py](file:///home/aellok/sovereign-mesh/train_sovereign.py):** Implements the PyTorch transformer, the **Factor-27 regularizer** (which penalizes neural path drift), and runs a local diagnostic training pass.
3.  **[gcp_deploy_gpu.sh](file:///home/aellok/sovereign-mesh/gcp_deploy_gpu.sh):** Automation script that enables GCP APIs, configures Artifact Registry, builds the container, pushes it, and provisions a GPU compute node running the container.
